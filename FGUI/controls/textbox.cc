...
void fgui::textbox::update() {
	...
	if (m_is_getting_key) {
		...
		for (std::size_t i = 0; i < fgui::key_status::KEY_MAX; i++) {
			...
			if (fgui::input.get_key_state(KEY_LSHIFT) || fgui::input.get_key_state(KEY_RSHIFT))
		...
		if (m_text.length() > 0) {
			...
			// 
			// TODO: Erase text when holding backspace
